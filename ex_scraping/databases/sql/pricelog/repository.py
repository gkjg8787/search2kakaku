...
class CategoryRepository(m_repository.ICategoryRepository):
    ...
    async def save_all(self, cate_entries: list[m_pricelog.Category]):
        ses = self.session
        adds = []
        updates = []
        for cate in cate_entries:
            result = await ses.execute(
                select(m_pricelog.Category)
                .where(m_pricelog.Category.category_id == cate.category_id)
                .where(m_pricelog.Category.entity_type == cate.entity_type)
            )
    ...
    async def get(
        ...
        ses = self.session
        stmt = select(m_pricelog.Category)
        if command.category_id:
            stmt = stmt.where(m_pricelog.Category.category_id == command.category_id)
        if command.name:
            stmt = stmt.where(m_pricelog.Category.name == command.name)
        if command.entity_type:
            stmt = stmt.where(m_pricelog.Category.entity_type == command.entity_type)
        result = await ses.execute(stmt)
